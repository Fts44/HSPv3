...
<main id="main" class="main">
    ...
    <x-admin.configuration.inventory.equipment-pagetitle activeTitle="item"></x-admin.configuration.inventory.equipment-pagetitle>
    <!-- Document Page Title -->
...
@push('script')
    ...
    <script>
        ...
        function update(item_id, item_name, item_unit, item_type, item_brand, item_category, item_status){
            clear();
            var url = "{{ route('AdminConfigruationUpdateEquipmentItem', ['id'=>'id']) }}";
            ...
            @if($errors->any())
                @if(session('status'))
                    ...
                    @if($status->action=='Add')
                        $('#form').attr('action', "{{ route('AdminConfigruationInsertEquipmentItem') }}");
