...
<main id="main" class="main">
    ...
    <x-admin.configuration.inventory.equipment-pagetitle activeTitle="type"></x-admin.configuration.inventory.equipment-pagetitle>
    <!-- Document Page Title -->
                        ...
                        <tr>
                            ...
                            @else
                                <td>
                                    ...
                                    <button class="btn btn-danger btn-sm" {{ ($item->ieid_id!=null) ? 'disabled' : '' }} onclick="return delete_confirmation('{{ $item->iet_type }}','{{ route('AdminConfigurationDeleteEquipmentType', ['id' => ($item->ieid_id!=null) ? 'id' : $item->iet_id ]) }}');"><i class="bi bi-eraser"></i></button>
                                </td>
                            @endif
                        </tr>
                    @endforeach
...
@push('script')
    ...
    <script>
        ...
        function update(item_id, item_name, item_status){
            clear();
            var url = "{{ route('AdminConfigruationUpdateEquipmentType', ['id'=>'id']) }}";
            ...
            @if($errors->any())
                @if(session('status'))
                    ...
                    @if($status->action=='Add')
                        $('#form').attr('action', "{{ route('AdminConfigruationInsertEquipmentType') }}");
                        $('#modal_title').html('Add Equipment Type');
                        $('#submit_button').html('Add');
                    @elseif($status->action=='Update')
                        $('#form').attr('action', "{{ route('AdminConfigruationUpdateEquipmentType', ['id' => $status->iet_id]) }}");
                        $('#modal_title').html('Update Equipment Type');
                        $('#submit_button').html('Update');
                    @endif
                @endif                
