...
<main id="main" class="main">
    ...
    <x-admin.configuration.inventory.equipment-pagetitle activeTitle="place"></x-admin.configuration.inventory.equipment-pagetitle>
    <!-- Document Page Title -->
...
@push('script')
    ...
    <script>
        ...
        function update(item_id, item_place, item_status){
            clear();
            var url = "{{ route('AdminConfigruationUpdateEquipmentPlace', ['id'=>'id']) }}";
            ...
            @if($errors->any())
                @if(session('status'))
                    ...
                    @if($status->action=='Add')
                        $('#form').attr('action', "{{ route('AdminConfigruationInsertEquipmentPlace') }}");
                        $('#modal_title').html('Add Equipment Place');
                        $('#submit_button').html('Add');
                    @elseif($status->action=='Update')
                        $('#form').attr('action', "{{ route('AdminConfigruationUpdateEquipmentPlace', ['id' => $status->iep_id]) }}");
                        $('#modal_title').html('Update Equipment Place');
                        $('#submit_button').html('Update');
                    @endif
                @endif                
