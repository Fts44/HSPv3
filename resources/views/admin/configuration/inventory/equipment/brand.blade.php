                        ...
                        <tr>
                            ...
                            @else
                                <td>
                                    ...
                                    <button class="btn btn-danger btn-sm" {{ ($item->ieid_id!=null) ? 'disabled' : '' }} href="" onclick="return delete_confirmation('{{ $item->ieb_brand }}','{{ route('AdminConfigurationDeleteEquipmentBrand', ['id' => ($item->ieid_id!=null) ? 'id' : $item->ieb_id ]) }}');"><i class="bi bi-eraser"></i></button>
                                </td>
                            @endif
                        </tr>
                    @endforeach
...
@push('script')
    ...
    <script>
        ...
        function update(item_id, item_name, item_status){
            clear();
            var url = "{{ route('AdminConfigruationUpdateEquipmentBrand', ['id'=>'id']) }}";
            ...
            @if($errors->any())
                @if(session('status'))
                    ...
                    @if($status->action=='Add')
                        $('#form').attr('action', "{{ route('AdminConfigruationInsertEquipmentBrand') }}");
                        $('#modal_title').html('Add Equipment Brand');
                        $('#submit_button').html('Add');
                    @elseif($status->action=='Update')
                        $('#form').attr('action', "{{ route('AdminConfigruationUpdateEquipmentBrand', ['id' => $status->ien_id]) }}");
                        $('#modal_title').html('Update Equipment Brand');
                        $('#submit_button').html('Update');
                    @endif
                @endif                
